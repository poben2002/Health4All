...
// Need to move this out and into a db eventually


const BreastCancerMap = () => {
    ...
    return (
        <MapContainer center={[47.608013, -122.335167]} zoom={10} style={{ height: "500px", width: "100%" }}>
          <TileLayer
            url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
            attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
          />
          {breastCancerData.map((location, index) => (
            <Marker key={index} position={[location.lat, location.lon]}>
              <Popup>
                <div>
                    <h3>{location.name}</h3>
                    <p>Breast Cancer Rate: {location.rate} per 100,000</p>
                    <p>Cases: {location.cases}</p>
                    <p>Comparison: {location.comparison}</p>
                </div>
              </Popup>
            </Marker>
          ))}
...
export default BreastCancerMap;
